...
def buy(symbol, time):
    ...
    try:
        ...
        if buy_ == False and status == True and pd.to_numeric(kilne_tracker[symbol].loc[length, 'Close']) < kilne_tracker[symbol].loc[length, coin_list[symbol]['type']]:
